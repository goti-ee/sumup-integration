defmodule SumupIntegration.Pipeline.SumupFeeReducerTest do
  ...
  describe "run/1" do
    test "deducts sumup fee from the amount" do
      transactions = [
        build(:sale_transaction, payment_method: :card, amount: 5.0, amount_gross: 5.0),
        build(:sale_transaction, payment_method: :card, amount: 12.589, amount_gross: 12.589),
        build(:sale_transaction, payment_method: :card, amount: 3.0, amount_gross: 3.0)
      ]
    ...
    test "ignores transactions where amounts are already different" do
      transaction =
        build(:sale_transaction, payment_method: :card, amount: 3.0, amount_gross: 5.0)

      assert [
               %SaleTransaction{amount: 3.0, amount_gross: 5.0}
